...
public class Player extends JPanel {
	...
	public ArrayList<Integer> getTitleDeeds() {
		return playerData.getTitleDeeds();
	}
	...
	public void withdrawFromWallet(int withdrawAmount) {
		if (withdrawAmount > playerData.getWallet()) {
			setVisible(false);
			System.out.println("Player " + playerNumber + " went bankrupt!");
		} else {
			playerData.setWallet(playerData.getWallet() - withdrawAmount);
		}
	}

	public void depositToWallet(int depositAmount) {
		playerData.setWallet(playerData.getWallet() + depositAmount);
		System.out.println("Payday for player " + getPlayerNumber() + ". You earned $200!");
	}

	public int getCurrentSquareNumber() {
		return playerData.getCurrentSquareNumber();
	}
	...
	public boolean hasTitleDeed(int squareNumber) {
		return playerData.getTitleDeeds().contains(squareNumber) ? true : false;
	}
	...
	public void buyTitleDeed(int squareNumber) {
		if (ledger.containsKey(squareNumber)) {
			...
			playerData.getTitleDeeds().add(this.getCurrentSquareNumber());
			ledger.put(squareNumber, this.getPlayerNumber()); // everytime a player buys a title deed, it is
																		// written in ledger, for example square 1
																		// belongs to player 2
	...
	public void move(int dicesTotal) {
		if (playerData.getCurrentSquareNumber() + dicesTotal > 19) {
			depositToWallet(200);
		}
		int targetSquare = (playerData.getCurrentSquareNumber() + dicesTotal) % 20;
		playerData.setCurrentSquareNumber(targetSquare);
